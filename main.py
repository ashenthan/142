...
#write the API for the popular articles
@app.route("/unliked-article", methods=["POST"])
def popular_article():
    article = all_articles[0]
...
#write the API for the recommemded articles
@app.route("/reccomended-article", methods=["POST"])
def popular_article():
    article = all_articles[0]
    reccomended_articles.append(article)
    all_articles.pop(0)
    return jsonify({
        "status": "success"
    }), 201
